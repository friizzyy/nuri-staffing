...
export function facilityNotification({ facilityName, contactName, email, phone, facilityType, location, beds, clinicianTypes, coverage, urgency, details }) {
  const urgencyBadge = urgency
    ? `<p style="margin:0 0 24px;">
        <span style="display:inline-block;background:${BRAND.roseLight};color:${BRAND.rose};font-family:${BRAND.font};font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;padding:4px 12px;border-radius:100px;">Urgency: ${urgency}</span>
      </p>`
    : '';

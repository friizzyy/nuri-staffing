 ...
 * Design system: Navy (#4A4063), Rose (#C2565E), Light BG (#F7F5F8)
    ...
    return `
      <tr>
        <td style="padding:16px 40px 20px;border-top:1px solid ${BRAND.borderLight};background-color:${BRAND.bg};">
          <p style="font-family:${BRAND.font};font-size:11px;color:${BRAND.textMuted};margin:0;line-height:1.6;text-align:center;">
...
export function cnaNotification({ firstName, lastName, email, phone, experience, licenseState, city, shiftPreference, settingPreference, heardAbout }) {
  const safeFirst = escapeHtml(capitalizeWords(firstName));
  const safeLast = escapeHtml(capitalizeWords(lastName));
  const safeEmail = escapeHtml(email);
  const formattedPhone = escapeHtml(formatPhone(phone));
  const content = `
    ${premiumHeader({ badge: 'New Application' })}
    <tr>
      <td style="padding:36px 40px 32px;">
        <h1 style="font-family:${BRAND.font};font-size:26px;font-weight:700;color:${BRAND.navy};margin:0 0 4px;letter-spacing:-0.03em;">${safeFirst} ${safeLast}</h1>
        <p style="font-family:${BRAND.font};font-size:13px;color:${BRAND.textMuted};margin:0 0 28px;">
          <a href="mailto:${safeEmail}" style="color:${BRAND.rose};text-decoration:none;">${safeEmail}</a>
          &nbsp;·&nbsp;
          <a href="tel:${escapeHtml(phone)}" style="color:${BRAND.rose};text-decoration:none;">${formattedPhone}</a>
        </p>
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-radius:12px;overflow:hidden;border:1px solid ${BRAND.border};">
          ${dataRow('Experience', experience ? escapeHtml(capitalizeWords(experience)) : 'Not specified')}
          ${dataRow('License', licenseState ? escapeHtml(capitalizeWords(licenseState)) : 'Not specified')}
          ${dataRow('City', city ? escapeHtml(capitalizeWords(city)) : 'Not specified')}
          ${dataRow('Shift', shiftPreference ? escapeHtml(capitalizeWords(shiftPreference)) : 'Not specified')}
          ${dataRow('Setting', settingPreference ? escapeHtml(capitalizeWords(settingPreference)) : 'Not specified')}
          ${dataRow('Source', heardAbout ? escapeHtml(capitalizeWords(heardAbout)) : 'Not specified', true)}
        </table>
      </td>
    </tr>
    ${premiumFooter({ compact: true })}`;

  return emailWrapper({ content, preheader: `New CNA application from ${safeFirst} ${safeLast}`, compact: true });
}
...
export function cnaConfirmation({ firstName }) {
  ...
  const content = `
    ...
    <tr>
      <td style="padding:36px 40px 32px;">
        <h1 style="font-family:${BRAND.font};font-size:26px;font-weight:700;color:${BRAND.navy};margin:0 0 6px;letter-spacing:-0.03em;">Welcome, ${safeFirst}!</h1>
...
export function facilityNotification({ facilityName, contactName, email, phone, facilityType, location, beds, clinicianTypes, coverage, urgency, details }) {
  const safeFacility = escapeHtml(capitalizeWords(facilityName));
  const safeContact = escapeHtml(capitalizeWords(contactName));
  const safeEmail = escapeHtml(email);
  const formattedPhone = escapeHtml(formatPhone(phone));
  const urgencyBadge = urgency
    ? `<p style="margin:0 0 24px;">
        <span style="display:inline-block;background:${BRAND.roseLight};color:${BRAND.rose};font-family:${BRAND.font};font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;padding:4px 12px;border-radius:100px;">Urgency: ${escapeHtml(capitalizeWords(urgency))}</span>
      </p>`
    : '';
  ...
  const detailsBlock = details
    ? `<div style="margin-top:20px;padding:16px 20px;background:${BRAND.bg};border-radius:10px;border-left:3px solid ${BRAND.rose};">
        ...
        <p style="font-family:${BRAND.font};font-size:14px;color:${BRAND.navy};line-height:1.65;margin:0;">${escapeHtml(details)}</p>
      </div>`
    : '';
  ...
  const content = `
    ...
    <tr>
      <td style="padding:36px 40px 32px;">
        <h1 style="font-family:${BRAND.font};font-size:26px;font-weight:700;color:${BRAND.navy};margin:0 0 4px;letter-spacing:-0.03em;">${safeFacility}</h1>
        <p style="font-family:${BRAND.font};font-size:13px;color:${BRAND.textMuted};margin:0 0 6px;">
          Contact: <strong style="color:${BRAND.navy};">${safeContact}</strong>
          &nbsp;·&nbsp;
          <a href="mailto:${safeEmail}" style="color:${BRAND.rose};text-decoration:none;">${safeEmail}</a>
          &nbsp;·&nbsp;
          <a href="tel:${escapeHtml(phone)}" style="color:${BRAND.rose};text-decoration:none;">${formattedPhone}</a>
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-radius:12px;overflow:hidden;border:1px solid ${BRAND.border};">
          ${dataRow('Type', facilityType ? escapeHtml(capitalizeWords(facilityType)) : 'Not specified')}
          ${dataRow('Location', location ? escapeHtml(capitalizeWords(location)) : 'Not specified')}
          ${dataRow('Beds', beds ? escapeHtml(beds) : 'Not specified')}
          ${dataRow('Clinicians', clinicianTypes ? escapeHtml(capitalizeWords(clinicianTypes)) : 'Not specified')}
          ${dataRow('Coverage', coverage ? escapeHtml(capitalizeWords(coverage)) : 'Not specified', true)}
        </table>
  ...
  return emailWrapper({ content, preheader: `New facility inquiry from ${safeFacility}`, compact: true });
}
...
export function facilityConfirmation({ contactName, facilityName }) {
  ...
  const content = `
    ...
    <tr>
      <td style="padding:36px 40px 32px;">
        <h1 style="font-family:${BRAND.font};font-size:26px;font-weight:700;color:${BRAND.navy};margin:0 0 6px;letter-spacing:-0.03em;">Thank you, ${safeContact}!</h1>
        <p style="font-family:${BRAND.font};font-size:15px;color:${BRAND.textMuted};line-height:1.75;margin:0 0 28px;">We've received your staffing inquiry for <strong style="color:${BRAND.navy};">${safeFacility}</strong>. A Nuri coordinator will follow up within <strong style="color:${BRAND.navy};">2 business hours</strong>.</p>
